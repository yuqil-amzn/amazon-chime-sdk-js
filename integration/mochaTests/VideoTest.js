const {
  ...
  ClickVideoButton,
  ClickVideoFilterButton,
  WaitForRemoteVideoCheckToComplete,
  WaitForRemoteParticipantsToTurnVideoOff,
  WaitForRemoteParticipantsToTurnVideoOn,
  WaitForRemoteParticipantsToJoinMeeting,
  WaitForMeetingToBeCreated,
} = require('./steps');
const {
...
class VideoTest extends SdkBaseTest {
  ...
  async runTestOnSingleSessions() {
    ...
    await test_window.runCommands(
      async () => await this.addUserToMeeting(test_attendee_id, session, useSimulcast)
    );
    await monitor_window.runCommands(
      async () => await this.addUserToMeeting(monitor_attendee_id, session, useSimulcast)
    );
    ...
    if (useVideoProcessor) {
      // TODO: Add step to check filter content
      await test_window.runCommands(
        async () => await ClickVideoFilterButton.executeStep(this, session)
      );
      await monitor_window.runCommands(
        async () => await ClickVideoFilterButton.executeStep(this, session)
      );
    ...
    await test_window.runCommands(
      async () => await LocalVideoCheck.executeStep(this, session, 'VIDEO_ON')
    );
    await monitor_window.runCommands(
      async () => await RemoteVideoCheck.executeStep(this, session, 'VIDEO_ON')
    );

    await test_window.runCommands(async () => await ClickVideoButton.executeStep(this, session));
    await test_window.runCommands(
      async () => await LocalVideoCheck.executeStep(this, session, 'VIDEO_OFF')
    );
    await monitor_window.runCommands(
      async () => await RemoteVideoCheck.executeStep(this, session, 'VIDEO_OFF')
    );

    // Check for unusual disconnections (don't need to wait here because we already did that earlier)
    await test_window.runCommands(
      async () => await RosterCheck.executeStep(this, session, 2, new RosterCheckConfig(2, 500))
    );
    await monitor_window.runCommands(
      async () => await RosterCheck.executeStep(this, session, 2, new RosterCheckConfig(2, 500))
    );
