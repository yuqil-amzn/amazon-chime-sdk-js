...
const {
  AppPage,
  MeetingReadinessCheckerPage,
  MessagingSessionPage,
  TestAppPage,
} = require('../pages');
    ...
    if (capabilities.video) {
      chromeOptions['args'].push(
        '--use-file-for-fake-video-capture=' +
          fetchMediaPath(capabilities.video, capabilities.browserName)
      );
    }
    if (capabilities.audio) {
      chromeOptions['args'].push(
        '--use-file-for-fake-audio-capture=' +
          fetchMediaPath(capabilities.audio, capabilities.browserName)
      );
    }
  }
...
const getPrerunScript = capabilities => {
  ...
  return name.includes(blurName) || name.includes(repName)
    ? 'storage:f196801d-998f-4ef2-bc2b-059b8521f345'
    : '';
};
...
class SaucelabsSession {
  ...
  async updateTestResults(passed) {
    ...
    console.log(
      `Publishing test results to saucelabs for session: ${sessionId} status: ${
